...
test('validateManifest accepts a color field inside a group', () => {
  assert.deepEqual(
    errsFor([{ key: 'slots', type: 'group', label: 'Slots', fields: [{ key: 'tint', type: 'color', label: 'Tint' }] }]),
    [],
  );
});
...
test('validateManifest applies the rule inside a group and inside an object', () => {
  ...
  const good = [
    typeField,
    { key: 'url', type: 'text', label: 'A', showIf: cond('a') },
    { key: 'url', type: 'text', label: 'B', showIf: cond('b') },
  ];
...
const withViews = views =>
  widgets.validateManifest('w', {
    name: 'w',
    label: 'W',
    sizes: ['small', 'medium'],
    viewField: 'view',
...
test('validateManifest accepts a picklist counted by size', () => {
  assert.deepEqual(
    errsFor([
      { key: 'bays', type: 'picklist', label: 'Bay', optionsFrom: 'devices', countBySize: { small: 4, medium: 10 } },
    ]),
    [],
  );
});
...
test('validateManifest rejects a showIf that is not an object', () => {
  for (const bad of [true, 'provider', 42, ['provider'], null]) {
    ...
    assert.ok(
      errs.some(e => /"showIf" that is not an object/.test(e)),
      JSON.stringify(bad),
    );
  }
});
...
test('validateManifest rejects a showIf with no condition', () => {
  ...
  assert.ok(
    errs.some(e => /"showIf" needs "equals" or "in"/.test(e)),
    errs.join('; '),
  );
});
...
test('validateManifest rejects an empty or non-array showIf.in', () => {
  for (const bad of [[], 'a', {}]) {
    ...
    assert.ok(
      errs.some(e => /"showIf\.in" must be a non-empty array/.test(e)),
      JSON.stringify(bad),
    );
  }
});
...
test('validateManifest rejects a showIf on the field itself', () => {
  ...
  assert.ok(
    errs.some(e => /"showIf" on itself/.test(e)),
    errs.join('; '),
  );
});
...
test('validateManifest rejects a group sub-field conditioned on a top-level field', () => {
  const errs = errsFor([
    typeField,
    {
      ...
      type: 'group',
      label: 'Services',
      fields: [{ key: 'url', type: 'text', label: 'U', showIf: { field: 'type', equals: 'a' } }],
    },
  ]);
  assert.ok(
    errs.some(e => /svcs: field "url" has a "showIf" on "type"/.test(e)),
    errs.join('; '),
  );
});
...
const withViewField = (viewField, fields, views) =>
  widgets.validateManifest('w', {
    name: 'w',
    label: 'W',
    sizes: ['small'],
...
test('validateManifest rejects a view no option can select', () => {
  ...
  assert.ok(
    errs.some(e => /view "map" cannot be selected/.test(e)),
    errs.join('; '),
  );
  assert.ok(
    errs.some(e => /offers "graph", which is not a declared view/.test(e)),
    errs.join('; '),
  );
});
