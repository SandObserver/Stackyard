...
- The dock is hidden when no app is docked, and the layout no longer reserves
  space for it.
...
- The analog clock's discs show the same overlap colours in every browser,
  instead of hiding each other and the date in Firefox.

- Settings now shows its confirmations, errors, pills, widget size names and
  connection-test results in the chosen language.

- Persian now uses one agreed word per concept for address, password, test and
...
- A volume mounted with root ownership no longer blocks config and icon writes.
...
- A password left stored while protection is off is now discarded on the next
  settings save.
...
- A badge extract path ending in `count` now reads a JSON field of that
  name. It only counts an array when the value is an array.
...
[1.8.0]: https://github.com/SandObserver/stackyard/compare/v1.7.1...v1.8.0
[1.7.1]: https://github.com/SandObserver/stackyard/compare/v1.7.0...v1.7.1
